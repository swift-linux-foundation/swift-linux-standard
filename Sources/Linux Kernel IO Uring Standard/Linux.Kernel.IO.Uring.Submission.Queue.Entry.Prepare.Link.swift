...
// ===----------------------------------------------------------------------===//
    ...
    extension Kernel.IO.Uring.Submission.Queue.Entry.Prepare {
        /// Hard link operation field accessors.
        ///
        /// Provides typed access to the SQE fields used by `IORING_OP_LINKAT`:
        /// AT_* flags (`rw_flags` as `hardlink_flags`), target directory fd (`len`),
        ...
        struct Link {
            ...
            /// Link AT_* flags (e.g., `AT_SYMLINK_FOLLOW`).
            @usableFromInline
        ...
        /// Access hard link operation fields.
        @inlinable
        var link: Link { unsafe Link(pointer: pointer) }
    }
